...
const LeaveApplicationValidation = require('../../controllers/Leave/leave.validator')
const LeaveApplicationHRValidation = require('../../controllers/Leave/leave.hr.validator')
...
router.put("/api/leave-application-emp/:id", verifyEmployee, (req, res) => {
    Joi.validate(req.body, LeaveApplicationValidation, (err, result) => {
        if (err) {
            console.log(err);
            res.status(400).send(err.details[0].message);
...
router.get("/api/leave-application-hr", verifyHR, (req, res) => {
    // var employee = {};
    // {path: 'projects', populate: {path: 'portals'}}
...
router.put("/api/leave-application-hr/:id", verifyHR, (req, res) => {
...
router.delete("/api/leave-application-hr/:id/:id2", verifyHR, (req, res) => {
    Employee.findById({ _id: req.params.id }, function (err, employee) {
        if (err) {
            res.send("error");
            console.log(err);
