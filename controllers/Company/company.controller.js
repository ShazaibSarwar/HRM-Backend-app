...
const CompanyValidation = require('../../controllers/Company/company.validator')


router.get("/api/company", verifyAdminHR, (req, res) => {
    // var employee = {};
...
router.post("/api/company", verifyHR, (req, res) => {
...
router.put("/api/company/:id", verifyHR, (req, res) => {
    Joi.validate(req.body, CompanyValidation, (err, result) => {
        if (err) {
            console.log(err);
            res.status(400).send(err.details[0].message);
